...
class StclairSpider(Spider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.start_urls = ["http://www.sheriff.co.st-clair.il.us/departments/sheriffs-department/corrections/inmate"]

    def parse(self, response):
        response.xpath('//input[@id="txtLastName"]')
        formid = "txtLastName"
        for formdata in [{formid: chr(x)} for x in range(97, 123)]:  # a-z
            yield FormRequest.from_response(response, formid=formid, formdata=formdata, callback=self.handle_form)

    def handle_form(self, form_response):
        table_rows = form_response.xpath('//table[@id="nameResultsTable"]').xpath("tr")
        for tr in table_rows[1:]:
            yield Request(
                url=tr.attrib["onclick"].split("showProgress();location.href=")[-1].strip("'"),
